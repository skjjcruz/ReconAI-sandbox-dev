...
Status: **superseded in the Yahoo initiation area; no deployment or public-ready proof**. Root found that the top-level cookie start was transferable. See [the correction and release gate](yahoo-initiation-binding-20260920.md); the original cookie-path closure claim below is historical and is not a current security conclusion. Frozen provider source commit `5d4bed8`. Worktree `/Users/jacobc/Projects/reconai-readiness-actual-proxies`, branch `codex/readiness-actual-proxies-20260920`, exact base `78294c80cf483aab32b37ae046d6b645f03ccb72`. This patch changes only provider functions/helpers, their tests, and the owning release prerequisites. Account/auth endpoints and account lifecycle source are untouched.
